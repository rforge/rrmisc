...
if (!file.exists(fn_ProfileData1)) {
  stop(paste("keine Datei 1 gefunden als:", fn_ProfileData1))
}
...
if (is.na(fn_ProfileData2) | fn_ProfileData2 == "NA") {
  ...
  i_ProfileData2 <- TRUE
  if (!file.exists(fn_ProfileData2)) {
    stop(paste("keine Datei 2 gefunden als:", fn_ProfileData2))
  }
}
#
# vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv
...
# xx xx xx xx xx xx xx xx xx xx xx xx xx xx xx xx xx xx xx xx xx xx xx xx xx xx xx xx xx xx xx xx xx
#
SwicaColsRGB <- list(c( 23, 159, 150),
                     c( 31, 195, 187),
                     c(  6,  73,  74),
                     c(129, 129, 129),
                     c(173, 173, 173),
                     c(255,   0,  68))
SwicaCols <- sapply(SwicaColsRGB, function(x) rgb(x[1], x[2], x[3], maxColorValue = 255))
#
...
#
# Temporären Ordner erstellen 'tempDirFigures' resp. Inhalt löschen                                      ..{{{
# xx xx xx xx xx xx xx xx xx xx xx xx xx xx xx xx xx xx xx xx xx xx xx xx xx xx xx xx xx xx xx xx xx
#
# tempDirFigures <- file.path(gsub("\\\\", "/", tempdir()), "Figures")
# if (dir.exists(tempDirFigures)) {
#   unlink(file.path(tempDirFigures, "*"))
# } else {
#   dir.create(tempDirFigures)
# }
# while ( tempDirFigures %in% dirs )
# {
#   tempDirFigures <- paste0(tempDirFigures, "A")
# }
#
# vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv
#}}}
...
if (LenCurAttr > 0) {
  ...
  for (j in 1:NPartsUebers)
  {
    ...
    # Dateiname aus tempfile() -- zugewiesen einer Variablen nach Inhalt der Grafik
    assign(paste0("PlotUebers", CurNpartsUebers), gsub("\\\\", "/", tempfile(fileext = ".png")))
    png(filename = get(paste0("PlotUebers", CurNpartsUebers))
...
if (LenCurAttr > 0) {
  ...
  for (j in 1:NPartsChar)
  {
    ...
    # Dateiname aus tempfile() -- zugewiesen einer Variablen nach Inhalt der Grafik
    assign(paste0("PlotChar", CurNpartsChar), gsub("\\\\", "/", tempfile(fileext = ".png")))
    png(filename = get(paste0("PlotChar", CurNpartsChar))
        , width = 9, height = GPars[1], units = "in"
        , res = 300)
    print(PlotChar)
    dev.off()
...
if (length(CurAttr0) > 0) {
  ...
  for (j in 1:NPartsNum)
  {
    ...
    # Dateiname aus tempfile() -- zugewiesen einer Variablen nach Inhalt der Grafik
    assign(paste0("PlotNum", CurNpartsNum), gsub("\\\\", "/", tempfile(fileext = ".png")))
    png(filename = get(paste0("PlotNum", CurNpartsNum))
        , width = 9, height = GPars[1], units = "in"
        , res = 300)
    print(PlotNum)
    dev.off()
...
if (length(CurAttr0) > 0) {
  ...
  for (j in 1:NPartsWerteBerNum)
  {
    ...
    # Dateiname aus tempfile() -- zugewiesen einer Variablen nach Inhalt der Grafik
    assign(paste0("PlotWerteBerNum", CurNpartsNum), gsub("\\\\", "/", tempfile(fileext = ".png")))
    png(filename = get(paste0("PlotWerteBerNum", CurNpartsNum))
        , width = 9, height = GPars[1], units = "in"
        , res = 300)
    print(PlotNum)
    dev.off()
...
if (length(CurAttr0) > 0) {
  ...
  for (j in 1:NPartsInt)
  {
    ...
    # Dateiname aus tempfile() -- zugewiesen einer Variablen nach Inhalt der Grafik
    assign(paste0("PlotInt", CurNpartsInt), gsub("\\\\", "/", tempfile(fileext = ".png")))
    png(filename = get(paste0("PlotInt", CurNpartsInt))
        , width = 9, height = GPars[1], units = "in"
        , res = 300)
    print(PlotInt)
    dev.off()
...
if (length(CurAttr0) > 0) {
  ...
  for (j in 1:NPartsWerteBerInt)
  {
    ...
    # Dateiname aus tempfile() -- zugewiesen einer Variablen nach Inhalt der Grafik
    assign(paste0("PlotWerteBerInt", CurNpartsInt), gsub("\\\\", "/", tempfile(fileext = ".png")))
    png(filename = get(paste0("PlotWerteBerInt", CurNpartsInt))
        , width = 9, height = GPars[1], units = "in"
        , res = 300)
    print(PlotInt)
    dev.off()
...
#  pfilen  <- file.path(tempDirFigures, "PlotWerteBerInt.png")
#  png(filename = pfilen
#      , width = 9, height = GPars[1], units = "in"
#      , res = 300)
...
if (length(CurAttr0) > 0) {
  ...
  for (j in 1:NPartsCharLikeNum)
  {
    ...
    # Dateiname aus tempfile() -- zugewiesen einer Variablen nach Inhalt der Grafik
    assign(paste0("PlotCharLikeNum", CurNpartsCharLikeNum), gsub("\\\\", "/", tempfile(fileext = ".png")))
    png(filename = get(paste0("PlotCharLikeNum", CurNpartsCharLikeNum))
        , width = 9, height = GPars[1], units = "in"
        , res = 300)
...
#  pfilen  <- file.path(tempDirFigures, "PlotCharLikeNum.png")
#  png(filename = pfilen
#      , width = 9, height = GPars[1], units = "in"
#      # , quality = 100
#      , res = 300)
...
if (length(CurAttr0) > 0) {
  ...
  for (j in 1:NPartsWerteBerCharLikeNum)
  {
    ...
    # Dateiname aus tempfile() -- zugewiesen einer Variablen nach Inhalt der Grafik
    assign(paste0("PlotWerteBerCharLikeNum", CurNpartsCharLikeNum), gsub("\\\\", "/", tempfile(fileext = ".png")))
    png(filename = get(paste0("PlotWerteBerCharLikeNum", CurNpartsCharLikeNum))
        , width = 9, height = GPars[1], units = "in"
        , res = 300)
    print(PlotCharLikeNum)
    dev.off()
...
#  pfilen          <- file.path(tempDirFigures, "PlotWerteBerCharLikeNum.png")
#  png(filename = pfilen
#      , width = 9, height = GPars[1], units = "in"
#      , res = 300)
#  print(PlotCharLikeNum)
...
if (length(CurAttr0) > 0) {
  ...
  for (j in 1:NPartsFactor)
  {
    ...
    # Dateiname aus tempfile() -- zugewiesen einer Variablen nach Inhalt der Grafik
    assign(paste0("PlotFactor", CurNpartsFactor), gsub("\\\\", "/", tempfile(fileext = ".png")))
    png(filename = get(paste0("PlotFactor", CurNpartsFactor))
        , width = 9, height = GPars[1], units = "in"
        , res = 300)
    print(PlotFactor)
    dev.off()
...
#  pfilen     <- file.path(tempDirFigures, "PlotFactor.png")
#  png(filename = pfilen
#      , width = 9, height = GPars[1], units = "in"
#      , res = 300)
#  print(PlotFactor)
...
if (length(CurAttr0) > 0) {
  ...
  for (j in 1:NPartsDate)
  {
    ...
    # Dateiname aus tempfile() -- zugewiesen einer Variablen nach Inhalt der Grafik
    assign(paste0("PlotDate", CurNpartsDate), gsub("\\\\", "/", tempfile(fileext = ".png")))
    png(filename = get(paste0("PlotDate", CurNpartsDate))
        , width = 9, height = GPars[1], units = "in"
        , res = 300)
    print(PlotDate)
    dev.off()
...
#  pfilen   <- file.path(tempDirFigures, "PlotDate.png")
#  png(filename = pfilen
#      , width = 9, height = GPars[1], units = "in"
#      , res = 300)
#  print(PlotDate)
...
if (length(CurAttr0) > 0) {
  ...
  for (j in 1:NPartsWerteBerDate)
  {
    ...
    # Dateiname aus tempfile() -- zugewiesen einer Variablen nach Inhalt der Grafik
    assign(paste0("PlotWerteBerDate", CurNpartsDate), gsub("\\\\", "/", tempfile(fileext = ".png")))
    png(filename = get(paste0("PlotWerteBerDate", CurNpartsDate))
        , width = 9, height = GPars[1], units = "in"
        , res = 300)
    print(PlotDate)
    dev.off()
...
#  pfilen   <- file.path(tempDirFigures, "PlotWerteBerDate.png")
#  png(filename = pfilen
#      , width = 9, height = GPars[1], units = "in"
#      , res = 300)
#  print(PlotDate)
